...
#include "thread.h"
#include <time.h>
#include "../../../sb7/sb7rstm/src/tm/tm_ptr.h"
#include "../../../sb7/sb7rstm/src/containers.h"
//#include <set>
...
void bench_init(void* _CFG)
{
  ...
  SET = sh_ptr<sb7::Set<uint64_t>>(new sb7::Set<uint64_t>());
  ...
  for (uint32_t w = 0; w < ((CFG_t*)CFG)->elements; w++){
      ...
      wr_ptr<sb7::Set<uint64_t>> wr(SET);
      wr->add(val);
...
void bench_test(void* _CFG)
{
  ...
  for(int i =0; i < CFG->runs; i++){
    ...
    if (act < CFG->lookpct) {
      ...
      //      sb7::SetIterator<sh_ptr<sb7::Set<int>> ret;
      bool ret;
      TM_BEGIN();
      //printf("look act %i\n", act);
      for(int ee =0; ee<ops; ee++){
	rd_ptr<sb7::Set<uint64_t>> rd(SET);
	ret = rd->contains(val[ee]);
	    //TMMAP_FIND(SET, val[ee]);
      }
      //            SET->lookup(val TM_PARAM);
      TM_END();
      lsum[tid] += ret;//(rdtsc()-lbegin);
    }
    else if (act < CFG->inspct) {
      //THREAD_MUTEX_LOCK(lock);        
      uint64_t ibegin = rdtsc();
      //std::pair<sb7::Set<int>::iterator,bool> ret;
      bool ret;
      TM_BEGIN();
      //printf("ins act %i\n", act);
      for(int ee =0; ee<ops; ee++){
...
MAIN(argc, argv)
{
  ...
  if(argc!=7){
    ...
    tls_array = (tls_t*)std::malloc(sizeof(tls_t)*numThread);
    int32_t numstart=0;
