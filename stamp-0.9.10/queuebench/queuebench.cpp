...
void bench_test(void* _CFG)
{
  ...
  for(int i =0; i < CFG->runs; i++){
    ...
    if (act < CFG->lookpct) {
      ...
      TM_BEGIN();
      //printf("ins act %i\n", act);
      result = TMQUEUE_PUSH(SET, &glob_mem[val]);
      TM_END();
	...
	tls_array[tid].inserts++;
      ...
      if(!result) printf("push not posssible \n");
    }
    else {
      //THREAD_MUTEX_LOCK(lock);        
      TM_BEGIN(/*atomic*/);
...
MAIN(argc, argv)
{
  ...
  if(argc!=6){
    printf("Usage: queuebenchSTM64 <1> <2> <3> <4> <5> where,\n<1>: Number of elements\n<2>: Lookup percentage\n<3>Insertion percentage\n<4>Iterations\n<5>Number of threads\n");
    abort();
  }
    CFG_t cfg ;
    printf("args %i\n", argc);
